  ...
  return (
    <div className="flex flex-col h-full bg-background-cream p-4 md:p-8">
       <header className="mb-6">
          <h1 className="text-4xl font-bold font-lora text-text-dark">Health Assistant</h1>
          <p className="mt-2 text-lg text-text-medium">Ask general questions about medications and services.</p>
        </header>
        ...
        <div className="flex-1 p-6 space-y-6 overflow-y-auto">
          {messages.map((message) => (
            <div key={message.id} className={`flex items-start gap-4 ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              {message.sender === 'ai' && (
                <div className="flex-shrink-0 h-10 w-10 rounded-full bg-primary-teal flex items-center justify-center text-white">
              ...
              <div className={`max-w-md p-4 rounded-2xl ${message.sender === 'user' ? 'bg-primary-teal text-white rounded-br-none' : 'bg-gray-100 text-gray-800 rounded-bl-none'}`}>
                 <p className="text-sm leading-6" style={{ whiteSpace: 'pre-wrap' }}>
                    {message.text}
                    {isLoading && message.sender === 'ai' && messages[messages.length-1].id === message.id && <span className="inline-block w-2 h-4 bg-gray-600 ml-1 animate-pulse"></span>}
                 </p>
              </div>
               {message.sender === 'user' && (
          ...
          {isLoading && messages[messages.length-1]?.sender === 'user' && (
            <div className="flex items-start gap-4 justify-start">
              ...
              <div className="max-w-md p-4 rounded-2xl bg-gray-100 text-gray-800 rounded-bl-none flex items-center space-x-2">
        ...
        <div className="p-4 bg-white border-t border-gray-200">
          <form onSubmit={handleSendMessage} className="relative">
            <input
              ...
              className="w-full pl-5 pr-14 py-3 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-primary-teal text-base"
              disabled={isLoading}
            />
            <button
              type="submit"
              disabled={isLoading || !input.trim()}
              className="absolute inset-y-1.5 right-1.5 flex items-center justify-center w-11 h-11 text-white bg-primary-teal rounded-full hover:bg-opacity-90 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
            >
