...
const NavLink: React.FC<{ to: string; children: React.ReactNode; isMobile?: boolean }> = ({ to, children, isMobile = false }) => {
  ...
  return (
    <Link
      to={to}
      className={` ${isMobile ? 'block w-full text-left px-4 py-3 text-base' : 'px-4 py-2 text-sm font-medium relative'} rounded-md transition-colors duration-200 ${isActive ? 'text-primary-teal' : 'text-gray-600 hover:text-primary-teal'}`}
    >
      {children}
      {isActive && !isMobile && (<span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-1/2 h-0.5 bg-primary-teal rounded-full"></span>)}
    </Link>
  );
};
    ...
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50 border border-gray-200">
          <button onClick={() => { navigate('/my-account'); setIsOpen(false); }} className="w-full text-left flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"><UserCircleIcon className="h-5 w-5 mr-2" /> My Account</button>
          <button onClick={() => { navigate('/order-history'); setIsOpen(false); }} className="w-full text-left flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"><ClipboardDocumentListIcon className="h-5 w-5 mr-2" /> Order History</button>
...
const Header: React.FC<HeaderProps> = ({ cartItemCount, onCartClick, user, onLogout }) => {
  ...
  const navItems = [
    { id: 'products', label: 'Wellness Shop', path: '/products' },
    { id: 'prescriptions', label: 'Prescriptions', path: '/prescriptions' },
    { id: 'pharmacist-messaging', label: 'Pharmacist Chat', path: '/pharmacist-messaging' },
    { id: 'health-hub', label: 'Health Hub', path: '/health-hub' },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <nav className="hidden md:flex md:space-x-1 lg:space-x-2">
            {user && navItems.map((item) => <NavLink key={item.id} to={item.path}>{item.label}</NavLink>)}
            {user?.role === 'admin' && <NavLink to="/admin/dashboard">Admin</NavLink>}
            {!user && <NavLink to="/products">Wellness Shop</NavLink>}
          </nav>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="hidden md:flex items-center">
              {user ? <UserMenu user={user} onLogout={onLogout} /> : <Link to="/login" className="flex items-center px-4 py-2 text-sm font-medium text-primary-teal bg-transparent border-2 border-primary-teal rounded-full hover:bg-primary-teal hover:text-white transition-colors">Sign In</Link>}
            </div>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-20 left-0 right-0 bg-white shadow-lg z-50 border-b border-gray-200" id="mobile-menu">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {user && <NavLink to="/dashboard" isMobile>My Health Hub</NavLink>}
            {user && navItems.map((item) => <NavLink key={item.id} to={item.path} isMobile>{item.label}</NavLink>)}
            {!user && <NavLink to="/products" isMobile>Wellness Shop</NavLink>}
            {user?.role === 'admin' && <NavLink to="/admin/dashboard" isMobile>Admin</NavLink>}
          </div>
          <div className="pt-4 pb-3 border-t border-gray-200">
            {user ? (
              <div className="mt-3 px-2 space-y-1">
                <button onClick={() => navigate('/my-account')} className="block w-full text-left px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:bg-gray-100">My Account ({user.email})</button>
              ...
              <div className="px-2"><Link to="/login" className="block w-full text-left px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:bg-gray-100">Sign In</Link></div>
            )}
          </div>
        </div>
      )}
