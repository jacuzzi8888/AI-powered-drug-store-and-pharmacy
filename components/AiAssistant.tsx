...
const AiAssistant: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-100 p-4 md:p-8">
       <header className="mb-4">
          <h1 className="text-3xl font-bold text-gray-900">Health Assistant</h1>
          <p className="mt-1 text-base text-gray-600">Ask general questions about medications and services.</p>
        </header>

      <div className="flex-1 flex flex-col bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden">
        <div className="flex-1 p-6 space-y-6 overflow-y-auto">
          {messages.map((message) => (
            <div key={message.id} className={`flex items-start gap-4 ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={`max-w-md p-4 rounded-2xl ${message.sender === 'user' ? 'bg-teal-600 text-white rounded-br-none' : 'bg-slate-200 text-gray-800 rounded-bl-none'}`}>
               ...
               {message.sender === 'user' && (
                <div className="flex-shrink-0 h-10 w-10 rounded-full bg-slate-300 flex items-center justify-center text-gray-600">
          ...
          {isLoading && messages[messages.length-1]?.sender === 'user' && (
            <div className="flex items-start gap-4 justify-start">
              ...
              <div className="max-w-md p-4 rounded-2xl bg-slate-200 text-gray-800 rounded-bl-none flex items-center space-x-2">
        ...
        <div className="p-4 bg-white border-t border-slate-200">
          <form onSubmit={handleSendMessage} className="relative">
            <input
              ...
              className="w-full pl-5 pr-14 py-3 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-teal-500 text-base"
              disabled={isLoading}
            />
            <button
              type="submit"
              disabled={isLoading || !input.trim()}
              className="absolute inset-y-1.5 right-1.5 flex items-center justify-center w-11 h-11 text-white bg-teal-600 rounded-full hover:bg-teal-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
            >
